...
void disastrOS_start(void (*f)(void*), void* f_args, char* logfile){  
  ...
  syscall_numarg[DSOS_CALL_SPAWN]      = 4;
...
void disastrOS_spawn(void (*f)(void*), void* args, float priority, float decay ) {
  long int prio = *(int*)(&priority);
  long int deca = *(int*)(&decay);
  disastrOS_syscall(DSOS_CALL_SPAWN, f, args, prio, deca);
}
