...
void internal_spawn(){
  ...
  float* dp_ptr=(float*)(&running->syscall_args[2]);
  new_pcb->priority_default   = *dp_ptr;
  float* tg_ptr=(float*)(&running->syscall_args[3]);
  new_pcb->priority_time_gain = *tg_ptr;
  new_pcb->priority_current = new_pcb->priority_default; 

  printf("created stuff %p pid: %d, prio: %f, tg: %f\n",
         new_pcb,
         new_pcb->pid,
         new_pcb->priority_default,
         new_pcb->priority_time_gain);
}
