...
typedef struct PCB{
  ...
  // robba aggiunta
  float priority_default;  // user defined priority at start  
  float priority_current;  // current priority changed depending on the status
  float priority_time_gain; // how much is the priority
                            // increasing each epoch a process remains in ready.
